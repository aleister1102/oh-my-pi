		...
		ViewportTailProvider
{
	...
	#replayPending = false;
	// Rows removed since the TUI last consumed the retirement report.
	...
	renderFullHistory(width: number): readonly string[] {
		const snapshot = new TranscriptContainer();
		for (const child of this.children) snapshot.addChild(child);
		return snapshot.render(width);
	}
	...
	override render(width: number): readonly string[] {
		...
		if (widthChanged) lines.length = 0;
		...
		if (this.#replayPending) {
			...
			// Committed rows arrive in the previous frame's coordinates. Wait for
			// post-emit publication before retiring rows laid out at a new width.
	...
	#compactCommittedPrefix(): void {
		...
		for (let i = this.#compactedChildStart; i < segments.length; i++) {
			...
			// Keep one visible block so a future append can derive its leading
			// separator from retained content instead of abutting native history.
			if (
		...
		let retainedIndex = dropUntil;
		while (retainedIndex < segments.length) {
			const candidate = segments[retainedIndex];
			if (candidate !== undefined && candidate.rowCount > 0) break;
			retainedIndex++;
		}
		const retained = segments[retainedIndex];
		if (dropRows > 0 && retained === undefined) {
			// A retained emitted row is the separator anchor for future appends.
			dropRows = 0;
			dropUntil = this.#compactedChildStart;
		} else if (retained !== undefined && retained.sep > 0) {
		...
		this.#retiredRowsPending += dropRows;
